...
//        }
    ...
    private void debugError(final Throwable th, PrintWriter printWriter) {
        ...
        AppHelper.print(printWriter.toString());
    }

    @SuppressLint("StaticFieldLeak")
    private void sendToEmail(final String stackTrace, final String fileName) {
        AppHelper.printLog("trying to send email!");
        new AsyncTask<Void, Void, Boolean>() {
            @Override
            protected Boolean doInBackground(Void... voids) {
                try {
                    AppHelper.printLog("Preparing crash report!");
                    List<String> toEmails = new ArrayList<>();
                    // TODO: 8/29/2019 remove at release
                    toEmails.add("mailto.developerkid@gmail.com");
                    // turn on low secure access on the sender email to receive crash report , visit the below page
                    // https://myaccount.google.com/security
                    GMail gMail = new GMail("inspiregeniussquad@gmail.com", "geniuspriya",
                            toEmails, ">----" + BuildConfig.APPLICATION_ID + " Crash Report----<",
                            stackTrace.replaceAll("\n", "<br>"));
                    gMail.createEmailMessage();
                    gMail.sendEmail();
                } catch (Exception e) {
                    e.printStackTrace();
                    AppHelper.printLog("GMail " + e.getMessage());
                    return false;
                }
                return true;
            }
